...
const run = async () => {
  ...
  const tokens = await getAnswer('How Many?? (y n)')

  // get nft price and approve contract for that amount
  const price = await contract.pricePerNft()
  const ext = price.mul(ethers.BigNumber.from(tokens))
  const tx1 = await cocoContract.connect(minter).approve(contract.address, ext)
  ...
  const tx = await contract.connect(minter).publicMintMulti(tokens)
